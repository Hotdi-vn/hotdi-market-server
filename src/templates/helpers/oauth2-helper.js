...
function fastifyOauth2 (fastify, options, next) {
  ...
  function generateAuthorizationUri (requestObject) {
    ...
    // added
    let redirectUri = callbackUri;
    if (requestObject.query.redirect_uri){
      redirectUri = requestObject.query.redirect_uri;
    }
    //
  ...
  const cbk = function (o, code, redirectUri, callback) {
  ...
  function getAccessTokenFromAuthorizationCodeFlowCallbacked (request, callback) {
    ...
    checkStateFunction(state, function (err) {
      ...
      cbk(fastify[name], code, redirectUri, callback)
    })
  }
